$(document).ready(function() {
    ...
    if (cart !== null)
        document.getElementById('cart-count').innerText = parseFloat(cart.length).toLocaleString('pt-BR')
    ...
    $('#order-tbl').find('th:nth-last-child(1), td:nth-last-child(1)').addClass('text-center')
});
...
function order_list_functionality() {
    onRowClick("orders_table", function(row) {
        ...
        if (user_is_super && row.classList.contains("mark-as-complete")) {
            var r = confirm("Gostaria de marcar o pedido " + id + " como entregue?");
            if (r == true) {
                $.ajax({
...
function display_notif(type, info = "No info provided") {
    ...
    switch (type) {
        case "add to cart":
            toastr.success(info.item_description + ': R$ ' + info.price, 'Adicionado ao Carrinho');
            break;
        case "remove from cart":
            toastr.info("Removido com sucesso " + info + " do carrinho");
            break;
        case "new order":
...
function load_cart() {
    ...
    if (cart !== null && cart.length > 0) {
        for (var i = 0; i < cart.length; i++) {
            ...
            item_price.innerHTML = "R$ " + parseFloat(cart[i].price).toLocaleString('pt-BR', { style: 'decimal', maximumFractionDigits: 2, minimumFractionDigits: 2 });

            total += cart[i].price
        }
        total = Math.round(total * 100) / 100
        localStorage.setItem('total_price', total);
        document.getElementById('total').innerHTML = "R$ " + parseFloat(localStorage.getItem("total_price")).toLocaleString('pt-BR', { style: 'decimal', maximumFractionDigits: 2, minimumFractionDigits: 2 })


        onRowClick("cart_body", function(row) {
            var value = row.getElementsByTagName("td")[0].innerHTML;
            var description = row.getElementsByTagName("td")[1].innerHTML;
            var r = confirm("Continuar para deletar '" + description + "' do carrinho?");
            if (r == true) {
...
function pizza_toppings(number_of_toppings, type_of_pizza, price) {
    ...
    $('#toppings_label')[0].innerHTML = "Escolha " + String(number_of_toppings) + " coberturas aqui"
    ...
    $("#submit_toppings").click(function() {
        ...
        var info = {
            "item_description": type_of_pizza + " Pizza com " + format_toppings(topping_choices),
            "price": price
        }
        add_to_cart(info)
...
function display_empty_cart() {
    ...
    document.getElementById('cart_heading').innerHTML = "O carrinho está vazio!"
...
function clearInput() {
    var getValue= document.getElementById("table");
        if (getValue.value !="") {
            getValue.value = "";
        }
}
...
function checkout() {
    ...
    $.ajax({
        ...
        type: "POST", // http method
        data: { cart: cart, price_of_cart: price_of_cart, table: table, csrfmiddlewaretoken: csrftoken }, // data sent with the post request
