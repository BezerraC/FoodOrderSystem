...
from . import forms
from .models import (Category, DinnerPlatters, Pasta, RegularPizza, Salad,
                     SavedCarts, SicilianPizza, Sub, Toppings, UserOrder)
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            return render(request = request,
                    ...
                    context={"form":form,"error":"Usuário ou senha incorretos"})
    form = forms.Login()
    return render(request = request,
                    template_name = "orders/login.html",
                    context={"form":form})
...
def checkout(request):
    if request.method == 'POST':
        ...
        order = UserOrder(username=username, order=list_of_items, price=float(price), table=table, delivered=False) #create the row entry
        order.save() #save row entry in database

        response_data['result'] = 'Pedido Recebido!'
        return HttpResponse(
